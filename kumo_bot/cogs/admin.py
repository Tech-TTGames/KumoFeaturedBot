...
class AdminCommands(commands.Cog):
    ...
    @app_commands.choices(mode=[
        ...
        app_commands.Choice(
            name="Modern+ (messages before vote, 10-25 required)", value=2),
    ])
    ...
    async def download(self, interaction: discord.Interaction,
        ...
        try:
            file = await asyncio.wait_for(downloaders.fetch_download(
                url, application, log_stuff),
                                          timeout=800)
