...
async def setup(ctx):
    ...
    await dm_channel.send(
        "Please provide the ID of the channel that will be used for the bot:"
    )
    while True:  # Get channel ID
        ...
        if chn.content.isnumeric():
            ...
            if channel is None:
                await dm_channel.send(
                    "That is not a valid channel ID or I am not present there."
                    "Please try again."
                )
            else:
    ...
    await dm_channel.send(
        "Please provide the ID of the role that will be used for the bot users:"
    )
    ...
    await dm_channel.send(
        "Please provide the ID of the role that will be mentioned during votes:"
    )
    while True:  # Get role ID
        rol = await bot.wait_for("message", check=dm_from_user)
        if rol.content.isnumeric():
